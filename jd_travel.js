...
async function dealHelpRes(functionId, inviteId, pin) {
    ...
    if (helpRes?.result?.score) {
        ...
        console.log(`互助成功，获得${score}个金币⭐️，他还需要${needNum}人完成助力，你还有${maxAssistTimes - alreadyAssistTimes}次助力机会`)
        if (!c) $.stopHelp = true
    } else {
...
function formatMsg(num, pre, ap) {
    console.log(`${pre ? pre + "：" : ""}获得${num}个金币⭐️${ap ? "，" + ap : ""}`)
}

function getSs(secretp) {
    $.random = Math.floor(1e7 + 9e7 * Math.random()).toString()
    $.sceneid = $.subSceneid ?? "CXJAssist_h5"
