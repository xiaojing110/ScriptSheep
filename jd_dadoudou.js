			...
			if ($.activityContent) {
				if (isdoTask) {
					...
					for (let i = 0; i < $.getProduct.length; i++) {
						await task('doTask', 'activityId=' + $.activityId + '&pin=' + encodeURIComponent($.secretPin) + '&taskId=followsku&param=' + $.getProduct[i].skuId);
					}
					await task('getProduct', 'type=1&activityId=' + $.activityId + '&pin=' + encodeURIComponent($.secretPin));
					for (let i = 0; i < $.getProduct.length; i++) {
						await task('doTask', 'activityId=' + $.activityId + '&pin=' + encodeURIComponent($.secretPin) + '&taskId=add2cart&param=' + $.getProduct[i].skuId);
					}
				}
				if (isplayGame) {
					$.stopGame = false;
...
function taskUrl(function_id, body, isCommon) {
	return {
        url: isCommon ? `https://cjhy-isv.isvjcloud.com/${function_id}` : `https://lzkj-isv.isvjcloud.com/wxgame/${function_id}`,
        headers: {
		...
		$.get({
			...
			try {
				if (e) {
					...
					if (resp.headers['set-cookie']) {
						cookie = originCookie + ';';
						for (let i of resp.headers['set-cookie']) {
							lz_cookie[i.split(';')[0].substr(0, i.split(';')[0].indexOf('='))] = i.split(';')[0].substr(i.split(';')[0].indexOf('=') + 1);
						}
						for (const vo of Object.keys(lz_cookie)) {
							cookie += (vo + '=' + lz_cookie[vo] + ';');
						}
