...
$.flCode=$.isNode()?process.env['FLCODNHJ']?process.env['FLCODNHJ']:'':'';
if ($.flCode=='') {
...
let cookiesArr=[];
if($.isNode()){
	Object.keys(jdCookieNode)['forEach'](_0x2252dd=>{
		cookiesArr.push(jdCookieNode[_0x2252dd]);
	});
	if(process.env['JD_DEBUG']&&process.env['JD_DEBUG']==='false')console.log=()=>{};
}else{
	cookiesArr=[$.getdata('CookieJD'),$.getdata('CookieJD2'),...$.toObj($.getdata('CookiesJD')||'[]')['map'](_0x1a2589=>_0x1a2589.cookie)]['filter'](_0x35eab6=>!!_0x35eab6);
}
let cookie='';
$.shareCode='';
!(async()=>{
	...
	const _0x219ed6=[''];
	for(let _0x5b560c=0;_0x5b560c<cookiesArr.length;_0x5b560c++){
		if(cookiesArr[_0x5b560c]){
			cookie=cookiesArr[_0x5b560c];
			$.UserName=decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/)&&cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
			$.index=_0x5b560c+1;
			$.isLogin=true;
			$.nickName='';
			flCode=$.flCode?$.flCode:_0x219ed6[random(0,_0x219ed6.length)];
	...
	$.log('','❌ '+$.name+', 失败! 原因: '+_0x5cbcaf+'!','');
})['finally'](()=>{
	$.done();
});
async function main(){
	$.code=flCode;
	let _0x1a5081=decodeURIComponent(cookie.match(/pt_pin=(.+?);/)&&cookie.match(/pt_pin=(.+?);/)[1]);
	...
	$.max=false;
	$.hotFlag=false;
	for(let _0x3425f8=0;_0x3425f8<3&&!$.max;_0x3425f8++){
		$.newCookie='';
		$.url1='';
		$.url2='';
		$.eid='';
		...
		await getInfo1();
		if(!$.url1){
			console.log(_0x1a5081+',初始化1失败,可能黑号');
			$.hotFlag=true;
			break;
		}
		await getInfo2();
		if(!$.url2){
			console.log(_0x1a5081+',初始化2失败,可能黑号');
			$.hotFlag=true;
			break;
		}
		$.actId=$.url2['match'](/mall\/active\/([^\/]+)\/index\.html/)&&$.url2['match'](/mall\/active\/([^\/]+)\/index\.html/)[1]||'31e6keDr2FdaUEVSvNZM2kjD7QVx';
		let _0x51127f=await getBody($.UA,$.url2);
		await getEid(_0x51127f);
		if($.eid){
			if(_0x3425f8===0&&$.shareCode){
...
function getEid(_0x14201f){
	return new Promise(_0x301232=>{
		const _0x1a9b12={'url':'https://gia.jd.com/fcf.html?a='+_0x14201f.a,'body':'d='+_0x14201f.d,'headers':{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','User-Agent':$.UA}};
		$.post(_0x1a9b12,async(_0x4e2eb1,_0x162797,_0x544269)=>{
			try{
				if(_0x4e2eb1){
					throw new Error(_0x4e2eb1);
				}else{
					if(_0x544269.indexOf('*_*')>0){
						_0x544269=_0x544269.split('*_*',2);
						_0x544269=JSON.parse(_0x544269[1]);
						$.eid=_0x544269.eid;
					}else{
...
function randomString(_0x15f0e5){
	_0x15f0e5=_0x15f0e5||32;
	let _0x3c743f='abcdef0123456789',_0x9f35bb=_0x3c743f.length,_0x1f6984='';
	for(i=0;i<_0x15f0e5;i++)_0x1f6984+=_0x3c743f.charAt(Math.floor(Math.random()*_0x9f35bb));
	return _0x1f6984;
}
function random(_0x8deeb4,_0x2db2c8){
	return Math.floor(Math.random()*(_0x2db2c8-_0x8deeb4))+_0x8deeb4;
}
async function getCoupons(_0x5af7b5){
	return new Promise(async _0x2103f4=>{
		const _0x406894={'platform':1,'unionActId':'31155','actId':$.actId,'d':$.code,'unionShareId':_0x5af7b5,'type':1,'eid':$.eid};
		const _0x367930={'appid':'u','functionId':'getCoupons','client':'apple','clientVersion':'8.3.6','body':_0x406894};
		const _0x36995e=await getH5st('6a98d',_0x367930);
		let _0x4a6853={'url':'https://api.m.jd.com/api?functionId='+_0x367930.functionId+'&appid='+_0x367930.appid+'&_='+Date.now()+'&loginType=2&body='+encodeURIComponent(JSON.stringify(_0x406894))+'&client='+_0x367930.client+'&clientVersion='+_0x367930.clientVersion+'&h5st='+encodeURIComponent(_0x36995e),'headers':{'Accept-Language':'zh-cn','Accept-Encoding':'gzip, deflate, br','Cookie':cookie+' '+$.newCookie,'user-agent':$.UA}};
		$.get(_0x4a6853,async(_0x4aa0a2,_0x2ec017,_0x54e381)=>{
			try{
				if(_0x4aa0a2){
					console.log(''+$.toStr(_0x4aa0a2));
					console.log($.name+' API请求失败，请检查网路重试');
				}else{
					let _0x578cab=$.toObj(_0x54e381,_0x54e381);
					if(typeof _0x578cab=='object'){
						if(_0x578cab.msg){
							console.log('结果：'+_0x578cab.msg);
							$.loginmsg=_0x578cab.msg;
						}
						if(_0x578cab.msg['indexOf']('上限')!==-1){
							$.max=true;
						}
						if($.shareId&&typeof _0x578cab.data!=='undefined'&&typeof _0x578cab.data['joinNum']!=='undefined'){
							console.log('当前'+_0x578cab.data['joinSuffix']+':'+_0x578cab.data['joinNum']);
						}
						if(_0x578cab.code==0&&_0x578cab.data){
							if(_0x578cab.data['type']==1){
								console.log('获得红包：'+_0x578cab.data['discount']+'元');
							}else if(_0x578cab.data['type']==3){
								console.log('获得优惠券：️满'+_0x578cab.data['quota']+'减'+_0x578cab.data['discount']);
							}else if(_0x578cab.data['type']==6){
								console.log('获得打折券：满'+_0x578cab.data['quota']+'打'+_0x578cab.data['discount']*10+'折');
							}else{
								console.log('获得未知'+(_0x578cab.data['quota']||'')+' '+_0x578cab.data['discount']);
								console.log(_0x54e381);
							}
						}
					}else{
						console.log(_0x54e381);
...
async function getInfo2(){
	return new Promise(_0x5624fe=>{
		const _0x47dce9={'url':$.url1,'followRedirect':false,'headers':{'Cookie':cookie+' '+$.newCookie,'user-agent':$.UA}};
		$.get(_0x47dce9,async(_0x338f42,_0x5d11cc,_0x3113cf)=>{
			try{
				let _0x1a25c3=_0x5d11cc&&_0x5d11cc.headers&&(_0x5d11cc.headers['set-cookie']||_0x5d11cc.headers['Set-Cookie']||'')||'';
				let _0x5232f1='';
				if(_0x1a25c3){
					if(typeof _0x1a25c3!='object'){
						_0x5232f1=_0x1a25c3.split(',');
					}else _0x5232f1=_0x1a25c3;
					for(let _0x1955fa of _0x5232f1){
						let _0x289dda=_0x1955fa.split(';')[0]['trim']();
						if(_0x289dda.split('=')[1]){
							if($.newCookie['indexOf'](_0x289dda.split('=')[1])==-1)$.newCookie+=_0x289dda.replace(/ /g,'')+'; ';
						}
					}
				}
				$.url2=_0x5d11cc&&_0x5d11cc.headers&&(_0x5d11cc.headers['location']||_0x5d11cc.headers['Location']||'')||'';
				$.url2=decodeURIComponent($.url2);
				$.url2=$.url2['match'](/(https:\/\/prodev\.m\.jd\.com\/mall[^'"]+)/)&&$.url2['match'](/(https:\/\/prodev\.m\.jd\.com\/mall[^'"]+)/)[1]||'';
			}catch(_0x360271){
				$.logErr(_0x360271,_0x5d11cc);
			}finally{
				_0x5624fe(_0x3113cf);
			}
		});
...
async function getInfo1(_0x23bd35){
	return new Promise(_0xafe850=>{
		const _0x2178d2={'url':'https://u.jd.com/'+$.code+'?s='+$.shareCode,'followRedirect':false,'headers':{'Cookie':_0x23bd35,'user-agent':$.UA}};
		$.get(_0x2178d2,async(_0x1bfe89,_0x26458d,_0x27736d)=>{
			try{
				let _0x2c032e=_0x26458d&&_0x26458d.headers&&(_0x26458d.headers['set-cookie']||_0x26458d.headers['Set-Cookie']||'')||'';
				let _0x5f27c9='';
				if(_0x2c032e){
					if(typeof _0x2c032e!='object'){
						_0x5f27c9=_0x2c032e.split(',');
					}else _0x5f27c9=_0x2c032e;
					for(let _0x53e013 of _0x5f27c9){
						let _0x4edec2=_0x53e013.split(';')[0]['trim']();
						if(_0x4edec2.split('=')[1]){
							if($.newCookie['indexOf'](_0x4edec2.split('=')[1])==-1)$.newCookie+=_0x4edec2.replace(/ /g,'')+'; ';
						}
					}
				}
				$.url1=_0x27736d.match(/(https:\/\/u\.jd\.com\/jda[^']+)/)&&_0x27736d.match(/(https:\/\/u\.jd\.com\/jda[^']+)/)[1]||'';
			}catch(_0xe2dc2){
				$.logErr(_0xe2dc2,_0x26458d);
			}finally{
				_0xafe850(_0x27736d);
			}
		});
...
async function loadH5Sdk(){
	const{JSDOM}=jsdom;
	let resourceLoader=new jsdom.ResourceLoader({'userAgent':'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:91.0) Gecko/20100101 Firefox/91.0','referrer':'https://msitepp-fm.jd.com/rest/priceprophone/priceProPhoneMenu'});
	let virtualConsole=new jsdom[('VirtualConsole')]();
	let options={'url':'https://msitepp-fm.jd.com/rest/priceprophone/priceProPhoneMenu','referrer':'https://msitepp-fm.jd.com/rest/priceprophone/priceProPhoneMenu','userAgent':'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:91.0) Gecko/20100101 Firefox/91.0','runScripts':'dangerously','resources':resourceLoader,'includeNodeLocations':true,'storageQuota':10000000,'pretendToBeVisual':true,'virtualConsole':virtualConsole};
