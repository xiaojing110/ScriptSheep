...
} else lliiii = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...IllIII($.getdata("CookiesJD") || "[]").map(IIll1 => IIll1.cookie)].filter(ll1il => !!ll1il);
...
function IllIII(l1iii) {
  if (typeof l1iii == "string") try {
    return JSON.parse(l1iii);
  } catch (lIIill) {
    return console.log(lIIill), $.msg($.name, "", "请勿随意在BoxJs输入框修改内容\n建议通过脚本去获取cookie"), [];
  }
}
// prettier-ignore
