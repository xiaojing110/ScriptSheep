...
function openAppUrl() {
	...
	if (url.substr(0, 5) === 'https') {
		...
		url2 = 'openApp.jdMobile://virtual?params=' + encodeURIComponent(JSON.stringify(_0x53f3b9));
		...
		url2 = 'openApp.jdMobile://virtual?params=' + encodeURIComponent(JSON.stringify(_0x1c5b66));
	}
	return url2;
}
		...
		$.get(options, async (err, resp, data) => {
			try {
				if (err) {
					...
					if (resp.status == 200) {
						let _0x38a232 = JSON.stringify(resp)
							.match(/LZ_TOKEN_KEY=[a-zA-Z0-9._-]+;/);
						let _0x1dccee = JSON.stringify(resp)
							.match(/LZ_TOKEN_VALUE=[\+a-zA-Z0-9._-]+/);
				...
				if (_0x4b830d) {
					...
					if (safeGet(_0x28a585)) {
						...
						if (_0x28a585.result && _0x28a585.data) {
							if (new Date(_0x28a585.data.active.endTimeStr.replace(/-/g, '/'))
								.getTime() < new Date()
								.getTime()) {
								$.toactivity = false;
								console.log('活动结束');
...
function joinTeam(_0x5d5fb6 = 0) {
	return new Promise(_0x3471e7 => {
		...
		$.post(taskPostUrl('/wxTeam/saveMember', _0x1bd3fe), async (e, err, data) => {
			try {
				if (e) {
					...
					if (safeGet(data)) {
						...
						if (data.result && data.data) {
							...
							if (data.errorMessage.indexOf('不是店铺会员') > -1 && _0x5d5fb6 != 3) {
								await joinShop();
								await joinTeam(3);
							} else if (data.errorMessage.indexOf('队伍已经满员') > -1) {
								$.maxTeam = true;
							} else if ((data.errorMessage.indexOf('奖品与您擦肩而过') > -1) && _0x5d5fb6 == 0) {
								await joinTeam(1);
							} else {
								console.log('异常7：' + JSON.stringify(data));
								message += '【京东账号' + $.index + '】 ' + data.errorMessage + '\n';
							}
						}
					}
				}
...
function taskPostUrl(_0x4e85b6, body) {
	return {
...
function safeGet(object) {
	try {
		if (typeof JSON.parse(object) == 'object') {
			return true;
		}
	} catch (e) {
		console.log(e);
...
function GetCookie() {
	if ($request.url.indexOf('/wxTeam/shopInfo') > -1) {
		if ($request.body) {
			...
			if (activityId) {
				let activityUrl = $request.url.split('/');
				console.log('activityId: ' + activityId[1]);
				console.log('activityUrl: ' + activityUrl[0] + '//' + activityUrl[2]);
				$.setdata(activityId[1], 'jd_smiek_zdjr_activityId');
				$.setdata(activityUrl[0] + '//' + activityUrl[2], 'jd_smiek_zdjr_activityUrl');
				$.msg($.name, '获取activityId: 成功🎉', 'activityId:' + activityId[1] + '\nactivityUrl:' + activityUrl[0] + '//' + activityUrl[2]);
			} else {
