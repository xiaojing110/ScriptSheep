...
if ($.isNode()) {
  ...
  if (process.env.jd_redrain_half_url) {
    jd_redrain_half_url = process.env.jd_redrain_half_url
    $.log(`jd_redrain_half_url \n ${jd_redrain_half_url}`)
  }
...
!(async () => {
  ...
  if (!redIds) {
    await $.wait(1000)
    redIds = await getRedRainIds('https://gitee.com/msewb/update/raw/master/m_redrain_half_5231520224612.json')
  }
  if (!redIds.length) {
    $.log(`\n今日龙王🐲出差，天气晴朗☀️，改日再来～\n`);
    return;
