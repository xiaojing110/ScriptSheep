...
!(async () => {
    if (!cookiesArr[0]) {
        ...
        await sendNotify.send(`${$.name}`, '【提示】请先获取京东账号一cookie\n直接使用NobyDa的京东签到获取');
        return;
    }
    // if (!activityIDList) {
    //     $.log(`没有通用ID任务，尝试获取远程`);
    ...
    //     }
    // }
    if (activityIDList && endTimeList && tasknameList) {
        message += `当前没有JoyJdOpen 环境变量`
        await notify.sendNotify(`${$.name}`, message);
        return
    }

    console.log(`通用ID任务就位，准备开始薅豆`);
    message += `activityIDList:${activityIDList}`
    message += `endTimeList:${endTimeList}`
    message += `tasknameList:${tasknameList}`
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            for (let j = 0; j < activityIDArr.length; j++) {
                ...
                message += `通用活动任务ID：${activityID}，结束时间：${endTime}，活动名称：${taskname}`
...
async function main () {
    ...
    do {
        ...
        console.log(`任务数量: ${$.dailyTaskList.length}\n任务列表：${activityID},获取成功`);
        ...
        for (let i = 0; i < $.dailyTaskList.length; i++) {
            ...
            if ($.oneTask.taskCount === $.oneTask.finishCount) {
                message += `groupType:${$.oneTask.groupType},已完成`;
                continue;
            }
