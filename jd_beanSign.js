...
if ($.isNode()) {
  ...
  I1liIll1 = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...iIlllIIi($.getdata("CookiesJD") || "[]").map(llI1I1I => llI1I1I.cookie)].filter(lii1 => !!lii1);
}
...
function lIl1I11i(I1iiiIl, IlIIi11l = {}) {
  ...
  if (IlIIi11l.sort) {
    i11II1i = i11II1i.sort();
  }

  for (let iiIilli1 of i11II1i) {
    let l1l1liIl = I1iiiIl[iiIilli1];

    if (l1l1liIl && typeof l1l1liIl === "object") {
      l1l1liIl = JSON.stringify(l1l1liIl);
    }

    if (l1l1liIl && IlIIi11l.encode) {
      l1l1liIl = encodeURIComponent(l1l1liIl);
    }
      ...
      try {
        if (lIi111) {
          ...
          if (IIIiiil1.code == 0 && !IIIiiil1?.["errorCode"]) {
            ...
            IIIiiil1.code == 3 ? console.log("签到失败," + IIIiiil1?.["errorMessage"]) : console.log("签到失败," + IIIiiil1?.["errorCode"] + ":" + IIIiiil1?.["errorMessage"]);
          }
        }
      } catch (ll1ii1lI) {
        $.log(ll1ii1lI);
...
async function I11i1I1() {
  ...
  i1llII = Buffer.from(ilIliil, "utf8").toString("base64");
