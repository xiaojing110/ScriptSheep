...
if ($.isNode()) {
  ...
  T = [$.getdata('CookieJD'), $.getdata('CookieJD2'), ...ad($.getdata('CookiesJD') || '[]').map(a => a.cookie)].filter(a => !!a);
}
...
!(async () => {
  ...
  $.log('默认前3个CK开团，调整设置变量DY_ZJDTOP ,定时自定义，一天三次');
  $.log('有问题TG喊我: https://t.me/dylan_jdpro\n');
  $.log('====================开始开团====================');
...
async function a2(a) {
  return h5st = await ae('vvipclub_distributeBean_assist', a, '03064'), new Promise(e => {
    $.post(a8(h5st.fn, h5st.body), async (g, h, i) => {
      try {
        if (g) {
          console.log('' + JSON.stringify(g)), console.log($.name + ' API请求失败，请检查网路重试');
        } else {
          if (ab(i)) {
            i = JSON.parse(i);
                ...
                if (i.resultCode === '9200011') {
                  ...
                  if (i.resultCode === '2400205') {
                    ...
                    if (i.resultCode === '90000014') {
                      $.log('助力结果：助力次数已耗尽'), $.canHelp = false;
                    } else {
                      if (i.resultCode === '2400203') {
                        $.log('助力结果：已助力过TA，还有助力次数');
                      } else {
...
async function a3() {
  return h5st = await ae('distributeBeanActivityInfo', {
    ...
    $.post(a8(h5st.fn, h5st.body), async (g, h, i) => {
      try {
        if (g) {
          console.log('' + JSON.stringify(g)), $.log('getUserTuanInfo API请求失败，请检查网路重试');
        } else {
          if (ab(i)) {
            i = JSON.parse(i);
            ...
            if (i.success) {
              ...
              if (i.data.assistStatus === 1 && !i.data.canStartNewAssist) {
                $.log('已开团，团成员人未满\n');
              } else {
                if (i.data.assistStatus === 2 && !i.data.canStartNewAssist) {
                  ...
                  if (i.data.assistStatus === 3 && i.data.canStartNewAssist) {
                    ...
                    i.data.assistStatus === 3 && !i.data.canStartNewAssist && $.log('今日开团已达上限，且当前团成员人已满\n');
                  }
                }
              }
              ...
              if (!i.data.canStartNewAssist && i.data.assistValidMilliseconds !== 0) {
                var k = {
                  activityIdEncrypted: i.data.id,
                  assistStartRecordId: i.data.assistStartRecordId,
                  assistedPinEncrypted: i.data.encPin,
                  channel: 'FISSION_BEAN',
                  launchChannel: ''
                };
                $.tuan = k;
              ...
              $.tuanActId = i.data.id;
              $.assistNum = i.data.assistNum || 4;
              $.assistStatus = i.data.assistStatus;
              $.canStartNewAssist = i.data.canStartNewAssist;
            } else {
              ;
              $.tuan = true, $.log('当前账号活动信息失败 ' + JSON.stringify(i) + '\n');
            }
          }
        }
      } catch (x) {
    ...
    $.post(a8(h5st.fn, h5st.body), async (f, g, h) => {
      try {
        if (f) {
          ...
          $.log('openTuan API请求失败，请检查网路重试');
        } else {
          if (ab(h)) {
            h = JSON.parse(h);
            h.success ? ($.log('>>>>>>>>>>>>>>>>>>>>>>>开团成功'), $.hasOpen = true) : console.log('\n开团失败：' + JSON.stringify(h) + '\n');
          }
        }
      } catch (q) {
        $.logErr(q, g);
...
async function ae(e, f, g) {
  ...
  $.ts = a6($.timestamp, 'yyyyMMddhhmmssSSS');
  $.fp = ac($.version);
  $.appId = g;
  $.dict = {
    [$.appId]: {}
  ...
  $.timestamp = new Date().getTime(), $.ts = a6($.timestamp, 'yyyyMMddhhmmssSSS');
  await a9($.appId);
  $.time1 = new Date().getTime();
  j = $.dict[$.appId];
  j.encrypt = await j.func(j.tk, $.fp, $.ts, $.appId, X).toString(X.enc.Hex);
  var s = {
    appid: 'vipMiddle',
    functionId: e,
  ...
  delete s.h5st;
  u = s;
  t = v.filter(F => u[F]).map(F => F + ':' + (F == 'body' ? X.SHA256(s[F]) : s[F])).join('&');
        ...
        if (j) {
          ...
          if (typeof m === 'object') {
            if (m.success && m.data) {
              ...
              if (Number(n.jdBeanQuantity) > 0) {
                beanNum += Number(n.jdBeanQuantity);
              }
              ...
              if (m.errorMessage) {
                if (m.errorMessage.indexOf('已签到') > -1 || m.errorMessage.indexOf('今天已经签到') > -1) {} else {
                  if (m.errorMessage.indexOf('进行验证') > -1) {
                    await injectToRequest('channelSign');
                  } else {
                    m.errorMessage.indexOf('火爆') > -1 && b == 2 ? await ag(a, 2) : console.log(turnTableId[a].name + ' ' + m.errorMessage);
                  }
                }
              } else {
    ...
    $.post(h, async (i, j, k) => {
      try {
        if (i) {
          ...
          k.indexOf('*_*') > 0 ? (k = k.split('*_*', 2), k = JSON.parse(k[1]), eid = k.eid) : console.log('京豆api返回数据为空，请检查自身原因');
        }
      } catch (l) {
        $.logErr(l, j);
      } finally {
