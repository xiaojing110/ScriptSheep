...
const endTime=1668182400000;
$.flCode=$.isNode()?process.env.FLCODETest?process.env.FLCODETest:'':'';
let cookiesArr=[];
if($.isNode()){
	Object.keys(jdCookieNode).forEach(_0x172a20=>{
		cookiesArr.push(jdCookieNode[_0x172a20]);
	});
	if(process.env.JD_DEBUG&&process.env.JD_DEBUG==='false')console.log=()=>{};
}else{
	cookiesArr=[$.getdata('CookieJD'),$.getdata('CookieJD2'),...$.toObj($.getdata('CookiesJD')||'[]').map(_0x17cad0=>_0x17cad0.cookie)].filter(_0x3d64da=>!!_0x3d64da);
}
let cookie='';
$.shareCode='';
!(async()=>{
	...
	const _0x342763=[''];
	for(let _0x52fefa=0;_0x52fefa<cookiesArr.length;_0x52fefa++){
		if(cookiesArr[_0x52fefa]){
			cookie=cookiesArr[_0x52fefa];
			$.UserName=decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/)&&cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
			$.index=_0x52fefa+1;
			$.isLogin=true;
			$.nickName='';
			flCode=$.flCode?$.flCode:_0x342763[random(0,_0x342763.length)];
	...
	$.log('','❌ '+$.name+', 失败! 原因: '+_0x454398+'!','');
}).finally(()=>{
	$.done();
});
async function main(){
	$.code=flCode;
	let _0x585a8b=decodeURIComponent(cookie.match(/pt_pin=(.+?);/)&&cookie.match(/pt_pin=(.+?);/)[1]);
	...
	$.max=false;
	$.hotFlag=false;
	for(let _0x1e1342=0;_0x1e1342<2&&!$.max;_0x1e1342++){
		$.newCookie='';
		$.url1='';
		$.url2='';
		$.eid='';
		...
		await getInfo1();
		if(!$.url1){
			console.log(_0x585a8b+',初始化1失败,可能黑号');
			$.hotFlag=true;
			break;
		}
		await getInfo2();
		if(!$.url2){
			console.log(_0x585a8b+',初始化2失败,可能黑号');
			$.hotFlag=true;
			break;
		}
		$.actId=$.url2.match(/mall\/active\/([^\/]+)\/index\.html/)&&$.url2.match(/mall\/active\/([^\/]+)\/index\.html/)[1]||'31e6keDr2FdaUEVSvNZM2kjD7QVx';
		console.log('$.actId:'+$.actId);
		let _0x81ee95=await getBody($.UA,$.url2);
		await getEid(_0x81ee95);
		if($.eid){
			if(_0x1e1342===0&&$.shareCode){
...
function getEid(_0x999d4){
	return new Promise(_0x263eac=>{
		const _0x3c6b4d={'url':'https://gia.jd.com/fcf.html?a='+_0x999d4.a,'body':'d='+_0x999d4.d,'headers':{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','User-Agent':$.UA}};
		$.post(_0x3c6b4d,async(_0x5446e3,_0x428097,_0x3c2979)=>{
			try{
				if(_0x5446e3){
					throw new Error(_0x5446e3);
				}else{
					if(_0x3c2979.indexOf('*_*')>0){
						_0x3c2979=_0x3c2979.split('*_*',2);
						_0x3c2979=JSON.parse(_0x3c2979[1]);
						$.eid=_0x3c2979.eid;
					}else{
...
function randomString(_0x3c31e3){
	_0x3c31e3=_0x3c31e3||32;
	let _0x4a9b65='abcdef0123456789',_0x180ad8=_0x4a9b65.length,_0x551f13='';
	for(i=0;i<_0x3c31e3;i++)_0x551f13+=_0x4a9b65.charAt(Math.floor(Math.random()*_0x180ad8));
	return _0x551f13;
}
function random(_0x442cee,_0x1c4eca){
	return Math.floor(Math.random()*(_0x1c4eca-_0x442cee))+_0x442cee;
}
async function getCoupons(_0x365936){
	return new Promise(async _0x5b4825=>{
		const _0x149651={'platform':1,'unionActId':'31149','actId':$.actId,'d':$.code,'unionShareId':_0x365936,'type':1,'eid':$.eid};
		const _0x331ec5={'appid':'u','functionId':'getCoupons','client':'apple','clientVersion':'8.3.6','body':_0x149651};
		const _0x5b70e1=await getH5st('6a98d',_0x331ec5);
		let _0x36ea48={'url':'https://api.m.jd.com/api?functionId='+_0x331ec5.functionId+'&appid='+_0x331ec5.appid+'&_='+Date.now()+'&loginType=2&body='+encodeURIComponent(JSON.stringify(_0x149651))+'&client='+_0x331ec5.client+'&clientVersion='+_0x331ec5.clientVersion+'&h5st='+encodeURIComponent(_0x5b70e1),'headers':{'Accept-Language':'zh-cn','Accept-Encoding':'gzip, deflate, br','Cookie':cookie+' '+$.newCookie,'user-agent':$.UA}};
		$.get(_0x36ea48,async(_0x5626d1,_0x4bbd36,_0x2f2008)=>{
			try{
				if(_0x5626d1){
					console.log(''+$.toStr(_0x5626d1));
					console.log($.name+' API请求失败，请检查网路重试');
				}else{
					let _0x15d3fe=$.toObj(_0x2f2008,_0x2f2008);
					if(typeof _0x15d3fe=='object'){
						if(_0x15d3fe.msg){
							console.log('结果：'+_0x15d3fe.msg);
							$.loginmsg=_0x15d3fe.msg;
						}
						if(_0x15d3fe.msg.indexOf('上限')!==-1){
							$.max=true;
						}
						if($.shareId&&typeof _0x15d3fe.data!=='undefined'&&typeof _0x15d3fe.data.joinNum!=='undefined'){
							console.log('当前'+_0x15d3fe.data.joinSuffix+':'+_0x15d3fe.data.joinNum);
						}
						if(_0x15d3fe.code==0&&_0x15d3fe.data){
							if(_0x15d3fe.data.type==1){
								console.log('获得红包：'+_0x15d3fe.data.discount+'元');
							}else if(_0x15d3fe.data.type==3){
								console.log('获得优惠券：️满'+_0x15d3fe.data.quota+'减'+_0x15d3fe.data.discount);
							}else if(_0x15d3fe.data.type==6){
								console.log('获得打折券：满'+_0x15d3fe.data.quota+'打'+_0x15d3fe.data.discount*10+'折');
							}else{
								console.log('获得未知'+(_0x15d3fe.data.quota||'')+' '+_0x15d3fe.data.discount);
								console.log(_0x2f2008);
							}
						}
					}else{
						console.log(_0x2f2008);
...
async function getInfo2(){
	return new Promise(_0x46358f=>{
		const _0x4e3bf0={'url':$.url1,'followRedirect':false,'headers':{'Cookie':cookie+' '+$.newCookie,'user-agent':$.UA}};
		$.get(_0x4e3bf0,async(_0x302b93,_0x4e9f59,_0x8670e6)=>{
			try{
				let _0x44943c=_0x4e9f59&&_0x4e9f59.headers&&(_0x4e9f59.headers['set-cookie']||_0x4e9f59.headers['Set-Cookie']||'')||'';
				let _0x29ef26='';
				if(_0x44943c){
					if(typeof _0x44943c!='object'){
						_0x29ef26=_0x44943c.split(',');
					}else _0x29ef26=_0x44943c;
					for(let _0x2997ca of _0x29ef26){
						let _0x33a336=_0x2997ca.split(';')[0].trim();
						if(_0x33a336.split('=')[1]){
							if($.newCookie.indexOf(_0x33a336.split('=')[1])==-1)$.newCookie+=_0x33a336.replace(/ /g,'')+'; ';
						}
					}
				}
				$.url2=_0x4e9f59&&_0x4e9f59.headers&&(_0x4e9f59.headers.location||_0x4e9f59.headers.Location||'')||'';
				$.url2=decodeURIComponent($.url2);
				$.url2=$.url2.match(/(https:\/\/prodev\.m\.jd\.com\/mall[^'"]+)/)&&$.url2.match(/(https:\/\/prodev\.m\.jd\.com\/mall[^'"]+)/)[1]||'';
			}catch(_0xa67368){
				$.logErr(_0xa67368,_0x4e9f59);
			}finally{
				_0x46358f(_0x8670e6);
			}
		});
...
async function getInfo1(){
	return new Promise(_0x1bbd9f=>{
		const _0x3f2cd8={'url':'https://u.jd.com/'+$.code+'?s='+$.shareCode,'followRedirect':false,'headers':{'Cookie':cookie,'user-agent':$.UA}};
		$.get(_0x3f2cd8,async(_0x5f57ac,_0x244682,_0xa078a6)=>{
			try{
				let _0x2be8af=_0x244682&&_0x244682.headers&&(_0x244682.headers['set-cookie']||_0x244682.headers['Set-Cookie']||'')||'';
				let _0x25c413='';
				if(_0x2be8af){
					if(typeof _0x2be8af!='object'){
						_0x25c413=_0x2be8af.split(',');
					}else _0x25c413=_0x2be8af;
					for(let _0x554baf of _0x25c413){
						let _0x3d352e=_0x554baf.split(';')[0].trim();
						if(_0x3d352e.split('=')[1]){
							if($.newCookie.indexOf(_0x3d352e.split('=')[1])==-1)$.newCookie+=_0x3d352e.replace(/ /g,'')+'; ';
						}
					}
				}
				$.url1=_0xa078a6.match(/(https:\/\/u\.jd\.com\/jda[^']+)/)&&_0xa078a6.match(/(https:\/\/u\.jd\.com\/jda[^']+)/)[1]||'';
			}catch(_0xb3adb7){
				$.logErr(_0xb3adb7,_0x244682);
			}finally{
				_0x1bbd9f(_0xa078a6);
			}
		});
