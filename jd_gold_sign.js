...
function goldCenterHead() {
    return new Promise(resolve => {
        ...
        $.get(options, async (err, resp, data) => {
            try {
                if (err) {
                    console.log('' + JSON.stringify(err)), console.log('goldCenterDoTask API请求失败，请检查网路重试');
                } else {
                    if (safeGet(data)) {
...
async function goldCreatorDoTask(body) {
    let log = await Faker.getBody('LogVM.js', 'coupon');
    log = {
        log: log.log,
        random: log.random,
