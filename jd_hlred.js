...
$.hlCode = $.isNode() ? process.env['HLCODE'] ? process.env['HLCODE'] : '' : '';
$.flCode = $.isNode() ? process.env['FLCODENHJ'] ? process.env['FLCODENHJ'] : '' : '';
if ($.flCode=='') {
    console.log('请填写FLCODENHJ环境变量,无内置码');
    return;
}
if ($.hlCode=='') {
    console.log('请填写HLCODE环境变量,无内置码');
    return;
}
let cookiesArr = [];
if ($.isNode()) {
    Object.keys(jdCookieNode)['forEach'](_0x5e48ea => {
        cookiesArr.push(jdCookieNode[_0x5e48ea]);
    });
    if (process.env['JD_DEBUG'] && process.env['JD_DEBUG'] === 'false') console.log = () => { };
} else {
    cookiesArr = [$.getdata('CookieJD'), $.getdata('CookieJD2'), ...$.toObj($.getdata('CookiesJD') || '[]')['map'](_0x5e0eda => _0x5e0eda.cookie)]['filter'](_0x46e92f => !!_0x46e92f);
}
let cookie = '';
$.shareCode = '';
!(async () => {
    ...
    for (let _0xabc414 = 0; _0xabc414 < cookiesArr.length; _0xabc414++) {
        if (cookiesArr[_0xabc414]) {
            cookie = cookiesArr[_0xabc414];
            $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
            $.index = _0xabc414 + 1;
            $.isLogin = true;
            $.nickName = '';
            owncode = _0x4c85b0[random(0, _0x4c85b0.length)];
            flCode = $.flCode ? $.flCode : owncode.flCodes;
            hlCode = $.hlCode ? $.hlCode : owncode.hlCodes;
    ...
    for (let _0x37a603 = 0; _0x37a603 < 1; _0x37a603++) {
        if (cookiesArr[_0x37a603]) {
            cookie = cookiesArr[_0x37a603];
            $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
            $.index = _0x37a603 + 1;
            $.isLogin = true;
            $.nickName = '';
    ...
    $.log('', '❌ ' + $.name + ', 失败! 原因: ' + _0x319310 + '!', '');
})['finally'](() => {
    $.done();
});
async function main() {
    $.code = flCode;
    let _0x11a464 = decodeURIComponent(cookie.match(/pt_pin=(.+?);/) && cookie.match(/pt_pin=(.+?);/)[1]);
    ...
    $.max = false;
    $.hotFlag = false;
    for (let _0x5bf30e = 0; _0x5bf30e < 10 && !$.max; _0x5bf30e++) {
        $.newCookie = '';
        $.url1 = '';
        $.url2 = '';
        $.eid = '';
        await getInfo1();
        if (!$.url1) {
            console.log(_0x11a464 + ',初始化1失败,可能黑号');
            $.hotFlag = true;
            break;
        }
        await getInfo2();
        if (!$.url2) {
            console.log(_0x11a464 + ',初始化2失败,可能黑号');
            $.hotFlag = true;
            break;
        }
...
async function main2() {
    $.code = flCode;
    let _0x228520 = decodeURIComponent(cookie.match(/pt_pin=(.+?);/) && cookie.match(/pt_pin=(.+?);/)[1]);
    $.UA = 'jdapp;iPhone;10.2.0;13.1.2;' + randomString(40) + ';M/5.0;network/wifi;ADID/;model/iPhone8,1;addressid/2308460622;appBuild/167853;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 13_1_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1;';
    $.max = false;
    $.hotFlag = false;
    for (let _0x4f2fa8 = 0; _0x4f2fa8 < 10 && !$.max; _0x4f2fa8++) {
        $.newCookie = '';
        $.url1 = '';
        $.url2 = '';
        $.eid = '';
        await getInfo1();
        if (!$.url1) {
            console.log(_0x228520 + ',初始化1失败,可能黑号');
            $.hotFlag = true;
            break;
        }
        await getInfo2();
        if (!$.url2) {
            console.log(_0x228520 + ',初始化2失败,可能黑号');
            $.hotFlag = true;
            break;
        }
        $.actId = $.url2['match'](/mall\/active\/([^\/]+)\/index\.html/) && $.url2['match'](/mall\/active\/([^\/]+)\/index\.html/)[1] || '31e6keDr2FdaUEVSvNZM2kjD7QVx';
    ...
    await getCoupons2($.shareCode);
}
function mainInfo() {
    return new Promise(_0x394914 => {
        let _0xc5e89f = { 'url': 'https://api.m.jd.com/api?functionId=shareUnionCoupon&appid=u&_=' + Date.now() + '&loginType=2&body=%7B%22unionActId%22%3A%2231155%22%2C%22actId%22%3A%22' + $.actId + '%22%2C%22platform%22%3A4%2C%22unionShareId%22%3A%22' + $.shareCode + '%22%2C%22d%22%3A%22' + $.code + '%22%2C%22supportPic%22%3A2%2C%22supportLuckyCode%22%3A0%2C%22eid%22%3A%22-1%22%7D&client=iPhone&clientVersion=&osVersion=iOS&screen=428*926&d_brand=iPhone&d_model=iPhone&lang=zh-CN&sdkVersion=&openudid=', 'headers': { 'Accept-Language': 'zh-cn', 'Accept-Encoding': 'gzip, deflate, br', 'Cookie': cookie + ' ' + $.newCookie, 'User-Agent': $.UA, 'referer': 'https://pro.m.jd.com/mall/active/3re5ajBZWA71ygjVnAz9kbaU1tfw/index.html?utm_term=f85312d60e0345e981d14923c7db8f66&topNavStyle=1&babelChannel=ttt3&utm_source=kong&hideTopFoot=1&utm_medium=jingfen&tttparams=%3D&cu=true&s=SCDSm&d=hdw7rfY&utm_campaign=&lat=&lng=&unionActId=31155' } };
        $.get(_0xc5e89f, async (_0x3702ad, _0x36cdda, _0x2ff2df) => {
            try {
                if (_0x3702ad) {
                    console.log('' + $.toStr(_0x3702ad));
                } else {
                    let _0x4715d1 = $.toObj(_0x2ff2df, _0x2ff2df);
                    if (typeof _0x4715d1 == 'object') {
                        if (_0x4715d1.code == 0 && _0x4715d1.data && _0x4715d1.data['shareUrl']) {
                            $.shareCode = _0x4715d1.data['shareUrl']['match'](/\?s=([^&]+)/) && _0x4715d1.data['shareUrl']['match'](/\?s=([^&]+)/)[1] || '';
                            console.log('助力码:' + $.shareCode);
                        }
                    } else {
                        console.log(_0x2ff2df);
                    }
                }
            } catch (_0x38d719) {
                $.logErr(_0x38d719, _0x36cdda);
            } finally {
                _0x394914();
            }
        });
...
function getEid(_0x958fbc) {
    return new Promise(_0x2e094f => {
        const _0x192b81 = { 'url': 'https://gia.jd.com/fcf.html?a=' + _0x958fbc.a, 'body': 'd=' + _0x958fbc.d, 'headers': { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8', 'User-Agent': $.UA } };
        $.post(_0x192b81, async (_0x94aea2, _0x1e29d7, _0x4193f0) => {
            try {
                if (_0x94aea2) {
                    throw new Error(_0x94aea2);
                } else {
                    if (_0x4193f0.indexOf('*_*') > 0) {
                        _0x4193f0 = _0x4193f0.split('*_*', 2);
                        _0x4193f0 = JSON.parse(_0x4193f0[1]);
                        $.eid = _0x4193f0.eid;
                    } else {
...
function randomString(_0x27fb75) {
    _0x27fb75 = _0x27fb75 || 32;
    let _0x58998a = 'abcdef0123456789', _0xf2b4e0 = _0x58998a.length, _0x4968a4 = '';
    for (i = 0; i < _0x27fb75; i++)_0x4968a4 += _0x58998a.charAt(Math.floor(Math.random() * _0xf2b4e0));
    return _0x4968a4;
}
async function getCoupons(_0x1d1d6e) {
    return new Promise(async _0x2709b5 => {
        const _0x12fd2b = { 'platform': 4, 'unionActId': '31155', 'actId': $.actId, 'd': $.code, 'unionShareId': _0x1d1d6e, 'type': 1, 'eid': $.eid };
        const _0x1f00a5 = { 'appid': 'u', 'functionId': 'getCoupons', 'client': 'iPhone', 'clientVersion': '8.3.6', 'body': _0x12fd2b };
        const _0x413e50 = await getH5st('6a98d', _0x1f00a5);
        let _0x535f92 = { 'url': 'https://api.m.jd.com/api?functionId=' + _0x1f00a5.functionId + '&appid=' + _0x1f00a5.appid + '&_=' + Date.now() + '&loginType=2&body=' + encodeURIComponent(JSON.stringify(_0x12fd2b)) + '&client=' + _0x1f00a5.client + '&clientVersion=' + _0x1f00a5.clientVersion + '&h5st=' + encodeURIComponent(_0x413e50), 'headers': { 'Accept-Language': 'zh-cn', 'Accept-Encoding': 'gzip, deflate, br', 'Cookie': cookie + ' ' + $.newCookie, 'user-agent': $.UA } };
        $.get(_0x535f92, async (_0x5f18a8, _0xf4d30, _0x17413e) => {
            try {
                if (_0x5f18a8) {
                    console.log('' + $.toStr(_0x5f18a8));
                    console.log($.name + ' API请求失败，请检查网路重试');
                } else {
                    let _0x382b5a = $.toObj(_0x17413e, _0x17413e);
                    if (typeof _0x382b5a == 'object') {
                        if (_0x382b5a.msg) {
                            console.log('结果：' + _0x382b5a.msg);
                        }
                        if (_0x382b5a.msg['indexOf']('上限') !== -1) {
                            $.max = true;
                        }
                        if ($.shareId && typeof _0x382b5a.data !== 'undefined' && typeof _0x382b5a.data['joinNum'] !== 'undefined') {
                            console.log('当前' + _0x382b5a.data['joinSuffix'] + ':' + _0x382b5a.data['joinNum']);
                        }
                        if (_0x382b5a.code == 0 && _0x382b5a.data) {
                            if (_0x382b5a.data['drawType'] == 1) {
                                msg = _0x382b5a.data['couponList'];
                                for (const _0x34c84d of msg) {
                                    if (_0x34c84d.type == 1) {
                                        console.log('获得红包：' + _0x34c84d.discount + '元');
                                    } else if (_0x34c84d.type == 3) {
                                        console.log('获得优惠券：️满' + _0x34c84d.quota + '减' + _0x34c84d.discount);
                                    } else if (_0x34c84d.type == 6) {
                                        console.log('获得打折券：满' + _0x34c84d.quota + '打' + _0x34c84d.discount * 10 + '折');
                                    } else {
                                        console.log('获得未知' + (voa.quota || '') + ' ' + _0x34c84d.discount);
                                        console.log(_0x17413e);
                                    }
                                }
                            }
                        }
...
async function getCoupons2(_0x31b987) {
    return new Promise(async _0x49108e => {
        const _0x247288 = { 'platform': 4, 'unionActId': '31155', 'actId': $.actId, 'd': $.code, 'unionShareId': _0x31b987, 'type': 2, 'eid': $.eid };
        const _0x288abc = { 'appid': 'u', 'functionId': 'getCoupons', 'client': 'iPhone', 'clientVersion': '8.3.6', 'body': _0x247288 };
        const _0x3450cd = await getH5st('6a98d', _0x288abc);
        let _0x24c91d = { 'url': 'https://api.m.jd.com/api?functionId=' + _0x288abc.functionId + '&appid=' + _0x288abc.appid + '&_=' + Date.now() + '&loginType=2&body=' + encodeURIComponent(JSON.stringify(_0x247288)) + '&client=' + _0x288abc.client + '&clientVersion=' + _0x288abc.clientVersion + '&h5st=' + encodeURIComponent(_0x3450cd), 'headers': { 'Accept-Language': 'zh-cn', 'Accept-Encoding': 'gzip, deflate, br', 'Cookie': cookie + ' ' + $.newCookie, 'user-agent': $.UA } };
        $.get(_0x24c91d, async (_0x2833ae, _0x8c36f3, _0x1c6088) => {
            try {
                if (_0x2833ae) {
                    console.log('' + $.toStr(_0x2833ae));
                    console.log($.name + ' API请求失败，请检查网路重试');
                } else {
                    let _0x1762e0 = $.toObj(_0x1c6088, _0x1c6088);
                    if (typeof _0x1762e0 == 'object') {
                        if (_0x1762e0.msg) {
                            console.log('结果：' + _0x1762e0.msg);
                        }
                        if (_0x1762e0.msg['indexOf']('上限') !== -1) {
                            $.max = true;
                        }
                        if ($.shareId && typeof _0x1762e0.data !== 'undefined' && typeof _0x1762e0.data['joinNum'] !== 'undefined') {
                            console.log('当前' + _0x1762e0.data['joinSuffix'] + ':' + _0x1762e0.data['joinNum']);
                        }
                        if (_0x1762e0.code == 0 && _0x1762e0.data) {
                            if (_0x1762e0.data['drawType'] == 1) {
                                msg = _0x1762e0.data['couponList'];
                                for (const _0x33bb4b of msg) {
                                    if (_0x33bb4b.type == 1) {
                                        console.log('获得红包：' + _0x33bb4b.discount + '元');
                                    } else if (_0x33bb4b.type == 3) {
                                        console.log('获得优惠券：️满' + _0x33bb4b.quota + '减' + _0x33bb4b.discount);
                                    } else if (_0x33bb4b.type == 6) {
                                        console.log('获得打折券：满' + _0x33bb4b.quota + '打' + _0x33bb4b.discount * 10 + '折');
                                    } else {
                                        console.log('获得未知' + (voa.quota || '') + ' ' + _0x33bb4b.discount);
                                        console.log(_0x1c6088);
                                    }
                                }
                            }
                        }
...
async function getInfo2() {
    return new Promise(_0x4ea050 => {
        const _0x51ad75 = { 'url': $.url1, 'followRedirect': false, 'headers': { 'Cookie': cookie + ' ' + $.newCookie, 'user-agent': $.UA } };
        $.get(_0x51ad75, async (_0x41b8af, _0x3b247f, _0xb6789a) => {
            try {
                let _0xecdb89 = _0x3b247f && _0x3b247f.headers && (_0x3b247f.headers['set-cookie'] || _0x3b247f.headers['Set-Cookie'] || '') || '';
                let _0xcf2ab0 = '';
                if (_0xecdb89) {
                    if (typeof _0xecdb89 != 'object') {
                        _0xcf2ab0 = _0xecdb89.split(',');
                    } else _0xcf2ab0 = _0xecdb89;
                    for (let _0x1beede of _0xcf2ab0) {
                        let _0x15ef2a = _0x1beede.split(';')[0]['trim']();
                        if (_0x15ef2a.split('=')[1]) {
                            if ($.newCookie['indexOf'](_0x15ef2a.split('=')[1]) == -1) $.newCookie += _0x15ef2a.replace(/ /g, '') + '; ';
                        }
                    }
                }
                $.url2 = _0x3b247f && _0x3b247f.headers && (_0x3b247f.headers['location'] || _0x3b247f.headers['Location'] || '') || '';
                $.url2 = decodeURIComponent($.url2);
                $.url2 = $.url2['match'](/(https:\/\/prodev\.m\.jd\.com\/mall[^'"]+)/) && $.url2['match'](/(https:\/\/prodev\.m\.jd\.com\/mall[^'"]+)/)[1] || '';
            } catch (_0x2435b3) {
                $.logErr(_0x2435b3, _0x3b247f);
            } finally {
                _0x4ea050(_0xb6789a);
            }
        });
...
async function getInfo1(_0x30022e) {
    return new Promise(_0x4faa29 => {
        const _0x3a91cc = { 'url': 'https://u.jd.com/' + $.code + '?s=' + $.shareCode, 'followRedirect': false, 'headers': { 'Cookie': _0x30022e, 'user-agent': $.UA } };
        $.get(_0x3a91cc, async (_0x408a82, _0x36d169, _0x5cf6c0) => {
            try {
                let _0x2afb48 = _0x36d169 && _0x36d169.headers && (_0x36d169.headers['set-cookie'] || _0x36d169.headers['Set-Cookie'] || '') || '';
                let _0x19f38f = '';
                if (_0x2afb48) {
                    if (typeof _0x2afb48 != 'object') {
                        _0x19f38f = _0x2afb48.split(',');
                    } else _0x19f38f = _0x2afb48;
                    for (let _0x2bc3f4 of _0x19f38f) {
                        let _0x179a5c = _0x2bc3f4.split(';')[0]['trim']();
                        if (_0x179a5c.split('=')[1]) {
                            if ($.newCookie['indexOf'](_0x179a5c.split('=')[1]) == -1) $.newCookie += _0x179a5c.replace(/ /g, '') + '; ';
                        }
                    }
                }
                $.url1 = _0x5cf6c0.match(/(https:\/\/u\.jd\.com\/jda[^']+)/) && _0x5cf6c0.match(/(https:\/\/u\.jd\.com\/jda[^']+)/)[1] || '';
            } catch (_0x4fd138) {
                $.logErr(_0x4fd138, _0x36d169);
            } finally {
                _0x4faa29(_0x5cf6c0);
            }
        });
...
function random(_0x498067, _0x41f11a) {
    return Math.floor(Math.random() * (_0x41f11a - _0x498067)) + _0x498067;
};
const navigator = {
